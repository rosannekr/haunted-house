...
// use something like figma to create radial gradient
...
// make sure the texture repeats instead of stretching
floorColorTexture.repeat.set(8, 8);
floorColorTexture.wrapS = THREE.RepeatWrapping;
floorColorTexture.wrapT = THREE.RepeatWrapping;

floorColorTexture.colorSpace = THREE.SRGBColorSpace; // always do this with color textures
...
roofColorTexture.wrapS = THREE.RepeatWrapping; // only need S bc repeat is only in x axis
...
const floor = new THREE.Mesh(
  new THREE.PlaneGeometry(20, 20, 100, 100), // increase vertices for more detailed displacement
  new THREE.MeshStandardMaterial({
    ...
    displacementScale: 0.3, // make it less bumpy
...
scene.add(house); // add objects that are part of the house to the house instead of the scene
...
walls.position.y += wallHeight / 2; // Move the walls up so they are on top of the floor
...
roof.position.y += wallHeight + roofHeight / 2; // Move the roof up so it is on top of the walls
...
bush1.rotation.x = -0.75; // hide weird spot in texture
...
for (let i = 0; i < 30; i++) {
  ...
  const radius = 3 + Math.random() * 4; // between 3 and 7 units from the center
  const x = Math.sin(angle) * radius;
  const z = Math.cos(angle) * radius;

  const grave = new THREE.Mesh(graveGeometry, graveMaterial);

  // position grave randomly in a circle around the house
  grave.position.x = x;
  grave.position.z = z;
  grave.position.y = Math.random() * 0.4;

  // subtract by 0.5 to get range between -0.5 and 0.5
  // for forward and backward rotation
  // and multiply by 0.4 to get a small rotation
 ...
 */
// Ambient light
// adding an ambient light in the same color makes it more realistic
...
// scene.fog = new THREE.Fog("#262837", 1, 15); // color, near, far
scene.fog = new THREE.FogExp2("#02343f", 0.1); // more realistic
...
const tick = () => {
  ...
  const ghost2Angle = -elapsedTime * 0.38; // invert direction and lower speed
